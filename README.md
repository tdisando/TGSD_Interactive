...
| Power law II | $$f(\phi) = \frac{1-\alpha}{\phi_{\max}^{1-\alpha}-\phi_{\min}^{1-\alpha}}\,\phi^{-\alpha}$$ | $\alpha$ = fractal exponent; $\phi_{\min}$ = coarse cutoff (largest clast, set by conduit or field observation); and $\phi_{\max}$ = fine cutoff (instrument resolution or aerodynamic settling limit) |
